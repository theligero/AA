...
public class MotionMoveMappedFunction : MotionMoveMappedBase
{
    ...
    public override void SetPositions(Vector3 a_origin, Vector3 a_destination, float dir)
    {
        ...
        _directionRotation90 = Utils.RotateVector2(_direction, _direction.x >= 0 ? 90 : -90, a_origin.z);
        ...
        if (speed is MotionAccelArriveSpeed)
            ((MotionAccelArriveSpeed)speed).SetDestination(Utils.IncPosition(this.transform.position, _direction.x, _direction.y, 0f));
        speed.StartAcceleration();
    }

    public override void SetDestination(Vector3 a_destination)
    {
        base.SetDestination(a_destination);
        moveDirection = 1f;
    }
    ...
    public override void Move(float time)
    {
        ...
        bool finish = false;
        if (moveDirection < 0f)
            _displacement = _displacement + _directionUnit * speed.CurrentSpeed * time * -1;
        else
            _displacement = _displacement + _directionUnit * speed.CurrentSpeed * time;

        float distance = Mathf.Abs(_displacement.x);
        float distanceToMax = Mathf.Abs(destination.x - (_origin + _displacement).x);
        Vector3 currentPosition = this.transform.position;
        if (moveDirection > 0f && distance < Mathf.Abs(_direction.x) || moveDirection < 0f && distanceToMax < Mathf.Abs(_direction.x))
        {
